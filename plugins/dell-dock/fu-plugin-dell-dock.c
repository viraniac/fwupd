...
fu_plugin_dell_dock_probe(FuPlugin *plugin, FuDevice *proxy, GError **error)
{
	...
	// is this needed?? FuContext *ctx = fu_plugin_get_context(plugin);
	...
	// is this needed?? fu_device_set_context(FU_DEVICE(mst_device), ctx);
	// is this needed?? fu_device_add_guid(FU_DEVICE(mst_device),
	// fwupd_guid_hash_string(instance));
	...
	// is this needed?? fu_device_set_context(FU_DEVICE(status_device), ctx);
	// is this needed?? fu_device_add_guid(FU_DEVICE(status_device),
	// fwupd_guid_hash_string(instance));
